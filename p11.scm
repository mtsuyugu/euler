...
(define (scan-one-row v)
 (let ((l (vector-length v)))
   (let loop ((i 0)
              (maxnum 0))
     (if (> i (- l 4)) 
       maxnum
       (loop (+ i 1) (max (fold * 1 (subseq v i (+ i 4))) maxnum))))))

(define (scan-rows-cols matrix)
  (let loop ((i 0)
             (maxnum 0))
    (let ((rowi (ref matrix i #f))
          (coli (list->vector (map (lambda (x) (ref x i #f)) matrix))))
      (if (not rowi)
        maxnum
        (loop (+ i 1) (max maxnum 
                           (scan-one-row rowi)
...
(define (scan-diagonal matrix)
  (let loop ((i 0)
             (j 0)
             (maxnum 0))
    (if (= i 17) maxnum
      (let ((diagij (filter values (map (let1 k j (lambda (x) (set! k (+ k 1)) (ref x (- k 1) #f)))
                                      (subseq matrix i)))))
        (if (null? diagij)
          maxnum
          (loop (if (= j 16) (+ i 1) i)
                (if (= j 16) 0 (+ j 1))
                (max maxnum (scan-one-row (list->vector diagij)))))))))

(define (scan-diagonal2 matrix)
  (let1 newmatrix (make-vector 20)
        ; make a symmetrical matrix and pass it to scan-diagonal
        (let loop ((i 0))
          (let1 rowi (ref matrix i #f)
                (if (not rowi)
                  newmatrix
                  (begin
                    (set! (ref newmatrix i) (vector-reverse-copy rowi))
                    (loop (+ i 1))))))
        (scan-diagonal newmatrix)))

(define (p11 matrix)
  (fold (lambda (x y) (let1 tmp (x matrix) (max tmp y))) 0 
        (list scan-rows-cols scan-diagonal scan-diagonal2)))
